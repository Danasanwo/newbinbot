...
async function indicate(historicalData) {
    ...
    try {
            ...
            const indicators = {
                SMA: {
                    today:  {
                        3: await SMA.calculate({period : 3, values : ohlcv.close}),
                        5: await SMA.calculate({period : 5, values : ohlcv.close}),
                        10: await SMA.calculate({period : 10, values : ohlcv.close}),
                        13: await SMA.calculate({period : 13, values : ohlcv.close}),
                        25: await SMA.calculate({period : 25, values : ohlcv.close}),
                        50: await SMA.calculate({period : 50, values : ohlcv.close}),
                        99: await SMA.calculate({period : 99, values : ohlcv.close}),
                    },
